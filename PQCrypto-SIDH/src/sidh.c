...
static void print_point(point_proj_t R)
{
    int i = 0;

    printf("\nX=0x");
    for (i = NWORDS_FIELD-1; i >= 0; --i) {
        printf("%016lx", R->X[0][i]);
    }
    printf(" + i*0x");
    for (i = NWORDS_FIELD-1; i >= 0; --i) {
        printf("%016lx", R->X[1][i]);
    }
    printf("\nZ=0x");
    for (i = NWORDS_FIELD-1; i >= 0; --i) {
        printf("%016lx", R->Z[0][i]);
    }
    printf(" + i*0x");
    for (i = NWORDS_FIELD-1; i >= 0; --i) {
        printf("%016lx", R->Z[1][i]);
    }
    printf("\n\n");
}

void custom_EphemeralKeyGeneration_A_SIDHp434(const unsigned char* PrivateKeyA)
{
    point_proj_t R, phiP = {0}, phiQ = {0}, phiR = {0}, pts[MAX_INT_POINTS_ALICE];
    f2elm_t XPA, XQA, XRA, coeff[3], A24plus = {0}, C24 = {0}, A = {0};
...
void custom_compute_guesses(unsigned char* guesses)
{
 // Alice's ephemeral public key generation with
    // (1) [2^eA-2]R0 = [2^eA-2](P + [0]Q) === [2^eA-2]R4 = [2^eA-2](P + [4]Q)
    // (2) [2^eA-2]R1 = [2^eA-2](P + [1]Q)
    // (3) [2^eA-2]R2 = [2^eA-2](P + [2]Q)
    // (4) [2^eA-2]R3 = [2^eA-2](P + [3]Q)
  // Input:  a private key PrivateKeyA in the range [0, 2^eA - 1]. 
  // Output: the public key PublicKeyA consisting of 3 elements in GF(p^2) which are encoded by removing leading 0 bytes.
    point_proj_t R, phiP = {0}, phiQ = {0}, phiR = {0}, pts[MAX_INT_POINTS_ALICE];
    f2elm_t XPA, XQA, XRA, A24plus = {0}, C24 = {0}, A = {0};
    int i, row, m, index = 0, pts_index[MAX_INT_POINTS_ALICE], npts = 0, ii = 0;
    digit_t SecretKeyA[NWORDS_ORDER] = {0};

    unsigned char PrivateKeyA[SIDH_SECRETKEYBYTES_A] = {0};

    for (i = 0; i < SIDH_SECRETKEYBYTES_A; ++i)
        PrivateKeyA[i] = 0x00;

    PrivateKeyA[0] = 0x03;
    PrivateKeyA[SIDH_SECRETKEYBYTES_A - 1] = 0x03;

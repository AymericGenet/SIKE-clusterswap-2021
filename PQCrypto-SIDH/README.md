...
The [PQCrypto-SIDH](https://github.com/microsoft/PQCrypto-SIDH) library was used to create valid random keys that were used to simulate a genuine communication between Alice (the laptop) and Bob (the attacked board) during the clustering power analysis.

## New additions

* [`custom folder`](custom/): Custom key generation code.

## Details

A file was separately created for each different instance:
- custom/custom-p434.c
- custom/custom-p503.c
- custom/custom-p610.c
- custom/custom-p751.c

Each of these files generate N random Bob secret keys and N random Alice public keys according to the PQCrypto-SIDH key generation procedure (using `crypto_kem_keypair_SIKEp<instance>` and `crypto_kem_enc_SIKEp<instance>` functions) that are then written in their respecive binary files `bob_sk_%05d.bin` and `alice_pk_%05d.bin` in `custom/data/p<instance>`.

---

The following is a copy of the README.md of Microsoft's PQCrypto-SIDH.

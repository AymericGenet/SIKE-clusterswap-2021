...
/* SIKEp610 constants */
#define SIKE_BOBSK3_P610_BYTES SECRETKEY_B_BYTES /* ((304 - 1 + 7) / 8) = 38 bytes */
#define SIKE_ALICEPK_P610_BYTES 486 /* 2*3*610 bits (R0->x[0], R0->x[1], R->x[0], R->x[1], R2->x[0], R2->x[1]) + 16 bytes (MSG) */
...
const digit_t custom_Montgomery_one[NWORDS_FIELD] = {
    0x670CC8E6, 0x00000000,
    0x00000000, 0x00000000,
    0x00000000, 0x00000000,
    0x00000000, 0x00000000,
    0x00000000, 0x9A340000,
    0x28717A3F, 0x4D99C2BD,
    0xA323D41C, 0x0A4A1839,
    0xD06AD1E2, 0xD2B62215,
    0x862CAF3D, 0x1369026E,
    0x0894E964, 0x00000001
};
...
uint8_t sk[SIKE_BOBSK3_P610_BYTES] = { 0x00 };
...
{ // Implement coordinate randomization
    ...
    fp2mul610_mont(R->X, rand_R, R->X);
    fp2mul610_mont(R->Z, rand_R, R->Z);
    fp2mul610_mont(R0->X, rand_R0, R0->X);
    fp2mul610_mont(R0->Z, rand_R0, R0->Z);
    fp2mul610_mont(R2->X, rand_R2, R2->X);
    fp2mul610_mont(R2->Z, rand_R2, R2->Z);
}
...
{ // Set private key involved in LADDER3PT
    for (int i=0; i < SIKE_BOBSK3_P610_BYTES; ++i)
    {
        sk[i] = k[i];
    }
    return 0x00;
...
{ // Initialize LADDER3PT without performing for-loop
    ...
    fpcopy610((digit_t*)&custom_Montgomery_one, A24[0]);
    fp2add610(A24, A24, A24);
    fp2add610(A, A24, A24);
    fp2div2_610(A24, A24);
    fp2div2_610(A24, A24);

    fpcopy610((digit_t*)&custom_Montgomery_one, (digit_t*)R2->Z);
    fpcopy610((digit_t*)&custom_Montgomery_one, (digit_t*)R->Z);
    fpcopy610((digit_t*)&custom_Montgomery_one, (digit_t*)R0->Z);
    fpzero610((digit_t*)(R->Z)[1]);
    fpzero610((digit_t*)(R0->Z)[1]);
    fpzero610((digit_t*)(R2->Z)[1]);
...
int main(void)
{
    ...
    /* Functions programmed to attack SIKEp610 */
    simpleserial_addcmd('s', SEED_BYTES, get_seed);
    simpleserial_addcmd('k', SIKE_BOBSK3_P610_BYTES, get_key);
    simpleserial_addcmd('p', SIKE_ALICEPK_P610_BYTES, get_pt);

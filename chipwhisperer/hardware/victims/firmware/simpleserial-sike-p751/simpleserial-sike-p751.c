...
/* SIKEp751 constants */
#define SIKE_BOBSK3_P751_BYTES SECRETKEY_B_BYTES /* ((378 - 1 + 7) / 8) = 48 bytes */
#define SIKE_ALICEPK_P751_BYTES 596 /* 2*3*751 bits (R0->x[0], R0->x[1], R->x[0], R->x[1], R2->x[0], R2->x[1]) + 16 bytes (MSG) */
...
const digit_t custom_Montgomery_one[NWORDS_FIELD] = {
    0x000249ad, 0x00000000,
    0x00000000, 0x00000000,
    0x00000000, 0x00000000,
    0x00000000, 0x00000000,
    0x00000000, 0x00000000,
    0x00000000, 0x83100000,
    0x375c6c66, 0x5527b1e4,
    0x3f4f24d0, 0x697797bf,
    0xac5c4e2e, 0xc89db7b2,
    0xd2076956, 0x4ca4b439,
    0x7512c7e9, 0x10f7926c,
    0x24bce5e2, 0x00002d5b
};
...
uint8_t sk[SIKE_BOBSK3_P751_BYTES] = { 0x00 };
...
{ // Implement coordinate randomization
    ...
    fp2mul751_mont(R->X, rand_R, R->X);
    fp2mul751_mont(R->Z, rand_R, R->Z);
    fp2mul751_mont(R0->X, rand_R0, R0->X);
    fp2mul751_mont(R0->Z, rand_R0, R0->Z);
    fp2mul751_mont(R2->X, rand_R2, R2->X);
    fp2mul751_mont(R2->Z, rand_R2, R2->Z);
}
...
{ // Set private key involved in LADDER3PT
    for (int i=0; i < SIKE_BOBSK3_P751_BYTES; ++i)
    {
        sk[i] = k[i];
    }
    return 0x00;
...
{ // Initialize LADDER3PT without performing for-loop
    ...
    fpcopy751((digit_t*)&custom_Montgomery_one, A24[0]);
    fp2add751(A24, A24, A24);
    fp2add751(A, A24, A24);
    fp2div2_751(A24, A24);
    fp2div2_751(A24, A24);

    fpcopy751((digit_t*)&custom_Montgomery_one, (digit_t*)R2->Z);
    fpcopy751((digit_t*)&custom_Montgomery_one, (digit_t*)R->Z);
    fpcopy751((digit_t*)&custom_Montgomery_one, (digit_t*)R0->Z);
    fpzero751((digit_t*)(R->Z)[1]);
    fpzero751((digit_t*)(R0->Z)[1]);
    fpzero751((digit_t*)(R2->Z)[1]);
...
int main(void)
{
    ...
    /* Functions programmed to attack SIKEp751 */
    simpleserial_addcmd('s', SEED_BYTES, get_seed);
    simpleserial_addcmd('k', SIKE_BOBSK3_P751_BYTES, get_key);
    simpleserial_addcmd('p', SIKE_ALICEPK_P751_BYTES, get_pt);
